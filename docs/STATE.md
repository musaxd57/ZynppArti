...
- **Moses adımları:** (1) `supabase/schema.sql`'i **tekrar çalıştır** (yeni share RPC'leri — idempotent). (2) Google OAuth'u Supabase'de aç (hâlâ "provider not enabled"). (3) Uçtan-uca tarayıcı testi: giriş→bulut kaydet/aç/**paylaş**→şifre sıfırla. (4) Branch'i main'e merge.
