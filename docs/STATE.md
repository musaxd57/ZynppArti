...
**Durum:** Üç sağlayıcı router (ADR-0042) **hız-öncelikli**: basit/orta→**OpenAI** (~1.3sn), karmaşık/yönetmelik→**Claude** (~2.7sn), **Akash yedek**. **Test: ~316.** 2026-06-24 turu **main'e MERGE edildi + push'landı** (canlıya deploy): YARIN §2 hızlı kazanımları + kapı/pencere incelemesi (kod doğru) + DWG canlı test + asistan meşgul göstergesi + doküman turu + **gizlilik politikası sayfası `/gizlilik`** (KVKK). Tarayıcı doğrulaması Moses'ta.
